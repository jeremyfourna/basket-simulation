...
function generateGame(style, teamsSpeed, team1, team2) {
  ...
  function generatePossession(team1, team2, history) {
    const shoot = typeOfShoot();
    const player = whoWillShoot(R.nth(R.prop('teamWithBall', history), [team1, team2]));
    const shootMade = willScore(shoot, R.path(['charact', charactForShoot(shoot)], player));

    if (R.equals(R.prop('remainingPossessions', history), 0)) {
      if (R.equals(needOvertime(history), true)) {
        return generatePossession(team1, team2, configForOvertime(history));
      } else {
        return history;
      }
      ...
      const transformations = {
        ...
        score: R.adjust(R.add(shoot), R.prop('teamWithBall', history)),
        teamWithBall: change,
      ...
      return generatePossession(team1, team2, R.evolve(transformations, history));
    } else {
      const transformations = {
        remainingPossessions: R.dec,
        teamWithBall: change,
  ...
  return generatePossession(team1, team2, history);
}
